# Script to import leads.sql file - drops only leads table if exists
...
Write-Host "Importing leads.sql (leads table only)" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
...
Write-Host "Step 1: Checking database..." -ForegroundColor Cyan
$checkDbCmd = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '$databaseName';"
if ($mysqlPassword) {
    $dbExists = & $mysqlPath -u $mysqlUser -p$mysqlPassword -e $checkDbCmd 2>&1
} else {
    $dbExists = & $mysqlPath -u $mysqlUser -e $checkDbCmd 2>&1
}

if ($dbExists -notmatch $databaseName) {
...
# Step 2: Drop only the leads table if it exists (preserves other tables)
Write-Host "Step 2: Dropping existing 'leads' table if it exists..." -ForegroundColor Cyan
$dropTableCmd = "DROP TABLE IF EXISTS `"$databaseName`".`leads`;"
if ($mysqlPassword) {
    & $mysqlPath -u $mysqlUser -p$mysqlPassword -e $dropTableCmd 2>&1 | Out-Null
} else {
    & $mysqlPath -u $mysqlUser -e $dropTableCmd 2>&1 | Out-Null
}
Write-Host "Leads table dropped (if it existed)." -ForegroundColor Green
Write-Host ""

# Step 3: Import the SQL file
Write-Host "Step 3: Importing leads.sql..." -ForegroundColor Cyan
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "Note: Other tables in the database were not affected." -ForegroundColor Yellow
    exit 1
}
